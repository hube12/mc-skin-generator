...
class FLAGS(object):
    def __init__(self):
        ...
        self.c_dim = 4       # "Number of image channels. [3]")
...
def get_celebA(output_size, n_epoch, batch_size):
    ...
    images_path = tl.files.load_file_list(path='../skins', regx='.*.jpg', keep_prefix=True, printable=False)
    def generator_train():
        for image_path in images_path:
            yield image_path.encode('utf-8')
    def _map_fn(image_path):
        image = tf.io.read_file(image_path)
        image = tf.image.decode_jpeg(image, channels=4)  # get RGB with 0~1
